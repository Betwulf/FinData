...
def RNN():
    ...
    # tf graph input
    x = tf.placeholder("float", [None, n_input_series, n_features])
    ...
    x = tf.reshape(x, [-1, n_input_series])

    # Generate a n_input-element sequence of inputs
    x = tf.split(x, n_input_series, 1)
    ...
    with tf.Session() as session:
        ...
        while step < epochs:
            ...
            _, acc, loss, latest_prediction = session.run([optimizer, accuracy, cost, prediction],
            ...
            if (step+1) % display_step == 0:
                ...
                acc_total = 0
                loss_total = 0
                ticker = descriptive_df['ticker'][-1]
                data_date = descriptive_df['date'][-1]
                print("Prediction for: {} - {}".format(ticker, data_date))
                print("Actual [%s] vs [%s]" % (label_data, latest_prediction))
            step += 1
